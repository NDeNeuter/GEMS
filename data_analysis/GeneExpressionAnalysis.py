...
class Preprocessor():
    ...
    def clean(self, df, rows_to_drop=['_unmapped', '_ambiguous', '_no_feature'], data_origin='GEMS'):
...
class LabelProcessor():
    ...
    def _make_label_df(self):
        ...
        self.label_df = pd.concat([self.reu_df, self.mening_df], sort=True)
    ...
    def add_samples(self, column, value):
        ...
        if self.internal_df is None:
            ...
            self.internal_df = pd.concat([self.internal_df, self.tmp_df], sort=True)
    ...
    def generate_DESeq_data(self, read_path='../data/readcounts_test.txt', col_data_path='../data/col_data_test.txt'):
        ...
        return self._generate_DESeq_data(self.internal_df, read_path, col_data_path)
    
    
    def generate_CV_DESeq_data(self, outputdir):
        
        outputpaths = []
        for ix in self.internal_df.index:
            
            # make dir to place output in
            ix_outdir = '{}/{}'.format(outputdir, ix)
            try:
                os.mkdir(ix_outdir)
            except:
                pass
            
            # select subparts of df by leaving one sample out
            not_ix_mask = self.internal_df.index != ix
            sub_df = self.internal_df.loc[not_ix_mask, :]
            
            # generate DESeq dataset for subdf
            read_path = ix_outdir+'/readcounts.txt'
            col_data_path = ix_outdir+'/col_data.txt'
            outputpaths.append(self._generate_DESeq_data(sub_df, read_path, col_data_path))
            
        return outputpaths
            
            
    def _generate_DESeq_data(self, df, read_path, col_data_path):
        
        gene_cols = [col for col in df.columns if 'gene' in col]
        meta_cols = [col for col in df.columns if 'gene' not in col]
        
        # gene read count
        read_df = df[gene_cols].T
        read_df.columns = df['name'].values
        read_df.rename_axis('genename', inplace=True)
        read_df.to_csv(read_path, sep='\t', index=True, header=True)
        
        # meta data
        col_data = df[meta_cols]
...
class DESeq2run():
    ...
    def _generate_R_script(self, design, contrast, contrast_subgroup1, contrast_subgroup2, R_script_dir, min_cum_reads):
        ...
        with open(script_path, 'w') as o:
            ...
            o.write('''
                    ...
                    '''.format(design, self.sample_column, min_cum_reads, self.test, self.data_dir,
    ...
    def PCA(self, color_by, show=False):
        ...
        if color_by != None:
            ...
            X = pd.concat([X, Y], ignore_index=True, axis=1, sort=True)
...
class DESeq2results():
    ...
    def heatmap(self, filename, genes='sig', readcountfile=None, coldatafile=None, color_label='group', label='group'):
        ...
        if genes == 'sig':
            sig_genes_df = self.deseq_result
            sig_genes = sig_genes_df.index
            X = norm_read_counts.loc[sig_genes, :].T
        if genes == 'all':
            X = norm_read_counts.T
        
        Y = meta_data[label]
        Y_color = meta_data[color_label]
    
        X = X.assign(index=pd.Series(Y).values).set_index('index')
        X.columns.names= ['genes']
        ...
        # fix color labels
        #colors = sns.diverging_palette(255, 133, l=60, n=len(Y_color.unique()), center="dark")
        colors = sns.hls_palette(n_colors=len(Y_color.unique()))
        color_dict = dict(zip(Y_color.unique(), colors))
        mapped_colors = [color_dict[x] for x in Y_color]
        
        # plot clustermap
        g = sns.clustermap(X.applymap(log), row_colors=mapped_colors, method='average', metric='euclidean')
        
        # add color label legend
        for color_l in Y_color.unique():
            g.ax_col_dendrogram.bar(0, 0, color=color_dict[color_l],
                                    label=color_l, linewidth=0)
        ...
        sns.set(font_scale=1) # set back to default value
